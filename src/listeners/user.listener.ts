...
export class UserListener {
  ...
  async handleDeliveryStaffRegistered(event: DeliveryStaffRegisteredEvent) {
    this.logger.log(`Handling delivery staff registration for: ${event.deliveryStaff.email}`);
    ...
    try {
      ...
      this.logger.log(`Delivery staff registration emails sent for ${event.deliveryStaff.email}`);
    } catch (error) {
      this.logger.error(`Failed to send delivery staff registration emails:`, error);
    }
  }
  ...
  async handleUserStatusUpdated(event: UserStatusUpdatedEvent) {
    ...
    try {
      ...
      this.logger.error(`Failed to send status update email to ${event.userEmail}:`, error);
    }
  }
}