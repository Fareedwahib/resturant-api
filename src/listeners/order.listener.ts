...
export class OrderListener {
  ...
  async handleOrderCreated(event: OrderCreatedEvent) {
    this.logger.log(
      `Handling order created event - Order: ${event.orderNumber}`,
    );

    try {
      await this.mailService.sendOrderConfirmationEmail(event.customerEmail, {
      ...
      this.logger.log(
        `Order confirmation email sent to ${event.customerEmail}`,
      );
    } catch (error) {
  ...
  async handleOrderStatusUpdated(event: OrderStatusUpdatedEvent) {
    ...
    try {
      await this.mailService.sendOrderStatusUpdateEmail(event.customerEmail, {
        orderNumber: event.orderNumber,
        customerName: event.customerName,
      ...
      this.logger.log(
        `Order status update email sent to ${event.customerEmail}`,
      );
    } catch (error) {
  ...
  async handleOrderCancelled(event: OrderCancelledEvent) {
    this.logger.log(
      `Handling order cancellation - Order: ${event.orderNumber}`,
    );

    try {
      await this.mailService.sendOrderCancellationEmail(event.customerEmail, {
        orderNumber: event.orderNumber,
        customerName: event.customerName,
        reason: event.reason,
      });
      ...
      this.logger.log(
        `Order cancellation email sent to ${event.customerEmail}`,
      );
    } catch (error) {
  ...
  async handleOrderPaymentUpdated(event: OrderPaymentUpdatedEvent) {
    ...
    try {
      if (event.paymentStatus === 'paid') {
        await this.mailService.sendPaymentConfirmationEmail(event.customerId, {
          orderNumber: event.orderNumber,
          totalAmount: event.totalAmount,
          paymentMethod: event.paymentMethod,
        ...
        await this.mailService.sendPaymentFailedEmail(event.customerId, {
          orderNumber: event.orderNumber,
          totalAmount: event.totalAmount,
          customerName: '',
        });
      ...
      this.logger.log(
        `Payment update email sent for order ${event.orderNumber}`,
      );
    } catch (error) {
